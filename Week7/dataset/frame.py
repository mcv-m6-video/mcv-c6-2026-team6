...
class ActionSpotDataset(Dataset):
    ...
    def __init__(
        ...
        # TGLS (soft labels around each action frame using a 1D Gaussian)
        self._tgls_enabled = tgls_enabled
        ...
        if tgls_enabled:
            ...
            kernel = np.exp(-0.5 * (offsets / max(tgls_sigma, 1e-6)) ** 2)
            kernel = kernel / kernel.sum()              # normalize
            self._tgls_kernel = kernel.astype(np.float32)
            self._tgls_offsets = offsets.astype(np.int32)
            print(f'[TGLS] enabled: sigma={tgls_sigma}, window={tgls_window}')
            print(f'[TGLS] kernel (normalized): {np.round(kernel, 4)}')
    ...
    def _get_one(self):
        ...
        # ── W7-3A: Temporal Gaussian soft labels ──────────────────────────
        if self._tgls_enabled and self._task == 'spotting':
            C = self._num_classes + 1
            soft = np.zeros((self._clip_len, C), dtype=np.float32)
            soft[:, 0] = 1.0                                      # default: background
            for label in dict_label:
                cls = int(label['label'])                         # class idx in [1..C-1]
                center = int(label['label_idx'])
                for off, w in zip(self._tgls_offsets, self._tgls_kernel):
                    t = center + int(off)
                    if t < 0 or t >= self._clip_len:
                        continue
                    # move "w" mass from background to this class at frame t
                    take = min(w, soft[t, 0])
                    soft[t, 0]   -= take
                    soft[t, cls] += take
            ret['soft_label'] = soft
