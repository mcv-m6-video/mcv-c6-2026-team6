...
def evaluate(model, dataset, batch_size=INFERENCE_BATCH_SIZE, nms_window=5,
             tolerances=(1.0, 0.5)):
    """
    W7-2: evaluate at multiple tolerances (in seconds).

    Returns:
        results: dict mapping "tol_<x>s" -> (mAP, AP_per_class)
        For backwards compatibility, also returns (mAP@first_tol, AP_per_class)
        as the first two elements of a tuple.
    """
    pred_dict = {}
    ...
    # Compute the performances at each tolerance
    results = {}
    for tol in tolerances:
        mAP, AP_per_class, _, _, _, _ = average_mAP(
            targets_numpy, detections_numpy, closests_numpy,
            FPS_SN / dataset._stride, deltas=np.array([tol])
        )
        results[f'tol_{tol}s'] = (mAP, AP_per_class)

    # Legacy return: (mAP, AP_per_class) at the first tolerance + full dict
    primary = results[f'tol_{tolerances[0]}s']
    return primary[0], primary[1], results
